  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-sans">Payments</h1>
          <p className="text-sm text-gray-500 mt-1">Total received: <span className="font-semibold text-green-700">{formatCurrency(Number(totalPaid._sum.amount || 0))}</span></p>
        </div>
        <Link href="/admin/payments/create" className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 shadow-sm">
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="p-5">
          <SearchFilter placeholder="Search by student, reference, or month..." />
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {payments.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{p.student.user.name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{p.enrollment.course.title}</td>
