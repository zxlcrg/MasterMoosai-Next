...
export default async function PaymentInvoicePage({ params }: Props) {
      ...
      <div className="invoice-print-area bg-white max-w-3xl mx-auto rounded-xl border border-gray-200 p-10 shadow-sm">
        ...
        <section className="grid grid-cols-2 gap-8 mb-6">
          <div>
            ...
            <p className="text-sm font-semibold text-gray-900">{payment.student.user.name}</p>
            <p className="text-xs text-gray-500">{payment.student.user.email}</p>
            {payment.student.user.phone && (
              <p className="text-xs text-gray-500">{payment.student.user.phone}</p>
            )}
          </div>
          <div className="text-right">
