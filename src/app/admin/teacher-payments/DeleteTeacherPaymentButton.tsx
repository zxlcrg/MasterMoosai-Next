...
import { deleteTeacherPayment } from "./actions";

export function DeleteTeacherPaymentButton({ id, label }: { id: number; label: string }) {
  ...
  async function handleDelete() {
    try {
      ...
      toast.success("Teacher payment deleted.");
      router.refresh();
    } catch (e: any) {
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Delete Teacher Payment</h3>
            <p className="text-sm text-gray-500 mb-6">Delete payment <span className="font-semibold">{label}</span>?</p>
